...
const Favourite = ({id_fav, dataStudy}) => {
  ...
  const deleteFavourite= async (id_fav)=>{
    try {
      ...
      setData((favs) =>
        favs.filter((fav) => fav.fav_id !== id_fav)
      );
      setFav(false);
    } catch (error) {
      console.error(error);
  ...
  const postFavourite = async (id_user, dataStudy) => {
    ...
    return (
        <>
        <svg xmlns="http://www.w3.org/2000/svg" onClick={handleClick} className="icon section icon_bookmark" id="Capa_2" data-name="Capa 2" viewBox="0 0 39.13 46.62">
  <g id="Capa_1-2" data-name="Capa 1">
    <g>
      <path className={`relleno ${fav ? "favorito" : ""}`} d="m2.56,44.14c-.14-15.92.02-32.28.17-35.87.04-.84.14-2.69,1.45-4.07,1.06-1.12,2.5-1.49,3.53-1.62h23.53c.49.02,2.05.15,3.44,1.35,1.73,1.49,1.91,3.51,1.94,3.91,0,10.56-.04,25.75-.04,36.31-5.69-4.47-11.39-8.93-17.08-13.4-5.65,4.47-11.3,8.93-16.95,13.4Z"/>
